...
// =============================================================================
// getnamecallmethod / setnamecallmethod — direct struct read via
// lua_State.namecall offset. Falls back to registry-cached name if the
// offset table is unpopulated (early boot before find_offsets ran).
// =============================================================================
extern "C" {
    // Populated by hooks.mm from the loaded offset table.
...
// =============================================================================
// getgc(includeTables?) — walk the Luau global GC page list when offsets
// are populated; fall back to registry-scan otherwise.
// =============================================================================
extern "C" {
...
static int lx_getgc(lua_State* S) {
    ...
    if (g_off_lua_state_global && g_off_global_allgcopages &&
        ...
        while (page && safety--) {
            ...
            if (want) {
                // Push the GCO as its typed value: safest path is
                // pushlightuserdata + let Lua-side code cast where needed.
